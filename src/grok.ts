...
function tryParsePosts(text: string): SignalPost[] {
  // Model is asked to return JSON; strip code fences if present and parse.
...
export async function grokSearchX(
  ...
  // Pull narrative either from the parsed JSON or fall back to the raw text.
  let narrative = "";
  try {
  ...
  const costUsd = usage?.cost_in_usd_ticks
    ? usage.cost_in_usd_ticks / 1_000_000_000  // ticks → USD (1 tick = 1e-9 USD)
    : undefined;
