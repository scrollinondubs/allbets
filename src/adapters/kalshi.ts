...
export class KalshiAdapter implements VenueAdapter {
  ...
  // per period. The series_ticker is the prefix-before-first-dash of the
  // market ticker (e.g. KXNEWPOPE-70-PPIZ → series=KXNEWPOPE).
  async getHistory(venueMarketId: string, range: HistoryRange): Promise<MarketHistory | null> {
    ...
    if (!seriesTicker) {
      return {
        ...
        series: [],
        stats: null,
        note: `cannot derive kalshi series_ticker from market ticker '${ticker}'`,
      };
    }
    ...
    const res = await fetch(url);
    if (!res.ok) {
      throw new Error(`kalshi candlesticks failed: ${res.status}`);
    }
    ...
    const series: HistoryPoint[] = (json.candlesticks ?? []).map((c) => {
      // Prefer mid of bid/ask close if available, fall back to previous price.
